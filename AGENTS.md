...
> **The compositor releases only the configs it made.** `gowl_compositor_set_config()` borrows: a config belongs to whoever gave it (`main()`, or an embedder such as cmacs), which releases it after the compositor. The `reload_config` action reads the file into a fresh config and keeps that one in `owned_config`; a later reload releases it once `set_config()` has moved off it, and finalize releases the last one. It used to release whatever `self->config` was -- usually the config it had been given -- which freed it under its owner: `set_config()` then disconnected a handler from the freed memory, and `main()` released it again at exit. Super+Shift+r is bound to it in the shipped config. `tests/test-keybind-dispatch.c` holds it with weak pointers.
