...
static const gchar glass_frag_src[] =
	...
	"  float ge  = 0.5;\n"
	"  vec2  g   = vec2(\n"
	"    sdf_rect(p + vec2(ge, 0.0), u_size, rd) - sdf_rect(p - vec2(ge, 0.0), u_size, rd),\n"
	"    sdf_rect(p + vec2(0.0, ge), u_size, rd) - sdf_rect(p - vec2(0.0, ge), u_size, rd));\n"
	"  float gl = length(g);\n"
	...
	"   * particular lens is. */\n"
	"  vec2 base = p + dir * m;\n"
	"  vec2 sep  = dir * (u_dispersion * ring);\n"
	"  vec3 c;\n"
