...
static const gchar water_frag_src[] =
	...
	"\n"
	"/*\n"
	" * Three iterations, two octaves each, the domain turned and doubled\n"
	" * between them.\n"
	" *\n"
	...
	" *\n"
	" * The turn between iterations is by an irrational-looking angle rather\n"
	" * than a right angle, so the cells of one octave never line up with\n"
	" * the cells of the next and the field has no visible grid.\n"
	" *\n"
	" * NOTHING HERE IS EVALUATED OUTSIDE A SINE.  That is what makes the\n"
	" * phase wrap in gowl_fx_water_advance() exact: every term is 2pi-\n"
	" * periodic in the phase, so a phase wrapped into [0, 2pi) draws the\n"
	" * identical surface.  An earlier version warped the domain with value\n"
	...
	"   * line up with the window's own edges and the surface reads as a\n"
	"   * grid laid on the screen rather than as water in it. */\n"
	...
	"\n"
	"  d  = sea_octave(uv + u_phase.x, choppy);\n"
	"  d += sea_octave(uv - u_phase.y, choppy);\n"
	"  h += d * amp; norm += 2.0 * amp;\n"
	"  uv = turn * uv; amp *= 0.30; choppy = mix(choppy, 1.0, 0.2);\n"
	"\n"
	"  d  = sea_octave(uv + u_phase.y, choppy);\n"
	"  d += sea_octave(uv - u_phase.z, choppy);\n"
	"  h += d * amp; norm += 2.0 * amp;\n"
	"  uv = turn * uv; amp *= 0.30; choppy = mix(choppy, 1.0, 0.2);\n"
	"\n"
	"  d  = sea_octave(uv + u_phase.z, choppy);\n"
	"  d += sea_octave(uv - u_phase.w, choppy);\n"
	"  h += d * amp; norm += 2.0 * amp;\n"
	"\n"
