...
static const gchar *default_yaml_config =
	...
	"sloppyfocus: true\n"
	"\n"
	"# Windows that get NO effects: no backdrop, no shadow, no open or\n"
	"# close animation.  Added to a built-in list -- steam,\n"
	"# steamwebhelper, mutter-devkit -- which cannot be configured\n"
	"# away, so games and a nested GNOME session need nothing here.\n"
	"# Matched against a window's app_id and against the command name\n"
	"# of its process and every ancestor, so naming a launcher covers\n"
	"# what it launches.  GOWL_NO_FX=1 in a process's environment does\n"
	"# the same for that process and its children, with no config.\n"
	"no-fx-apps: \"\"\n"
	"\n"
	"# Palette -- one set of colours for the whole session.  Every colour\n"
	"# setting below may be a literal, a name from this palette, or\n"
	"# \"name/aa\" for the same colour at a different opacity.\n"
	"# Flavours: mocha, macchiato, frappe, latte, dwm.\n"
	"palette:\n"
	"  name: mocha\n"
	"\n"
	"# Appearance\n"
	...
	"tag-count: 9\n"
	"\n"
	"# Motion.  Only POSITION is animated; see the manual for why.\n"
	"# Curves: linear, ease-out-quint, ease-in-out-cubic,\n"
	"#         almost-linear, quick.\n"
	"animations: true\n"
	"animation-duration: 150\n"
	"animation-duration-open: 190\n"
	"animation-duration-close: 120\n"
	"# ease-out-back and spring overshoot; the rest do not.\n"
	"animation-curve: \"ease-out-expo\"\n"
	"\n"
	"# Scrolling layout column width, as a fraction of usable width.\n"
	...
	"  # floating are what you reach for mid-task; changing the tag's\n"
	"  # layout by accident while looking for either is the more\n"
	"  # expensive mistake, and a tag that has quietly gone to the float\n"
	"  # layout looks like the tiling broke.\n"
	"  #\n"
	"  # Floating a window takes it out of the tiling and re-tiles what\n"
	"  # is left; unfloating puts it back into the current tag's layout.\n"
	"  \"Super+f\": { action: toggle_fullscreen }\n"
	"  \"Super+Shift+f\": { action: toggle_float }\n"
	"\n"
	"  # A floating window normally sits ON TOP of the tiling.  These\n"
	"  # push it behind instead, so the tiles underneath can be worked\n"
	"  # on without moving or closing it; the same key brings it back,\n"
	"  # and it keeps the keyboard while it is down.  The second key is\n"
	"  # every floating window on the screen at once.\n"
	"  \"Super+Alt+b\": { action: toggle_below }\n"
	"  \"Super+Ctrl+Alt+b\": { action: toggle_below_all }\n"
	"\n"
	"  # Layout selection.  The float LAYOUT -- every window floating at\n"
	"  # once -- has no key here; it is reached by cycling, like the\n"
	"  # eight other layouts that never had one.\n"
	...
	"\n"
	"  # Media, volume and brightness keys.  These take no modifier;\n"
	"  # dispatch matches a cleaned modifier mask, so zero is fine.\n"
	"  # wpctl comes with wireplumber; brightnessctl and playerctl are\n"
	"  # separate packages and these binds are inert without them.\n"
