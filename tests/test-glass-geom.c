 ...
 *
 * Where a window's glass is rendered, and what part of it shows.
 *
 ...
 *
 * The one that merely looks wrong: the glass works in three coordinate
...
#include "../modules/liquidglass/gowl-glass-geom.c"

/* The exact condition wlroots asserts. */
static void
assert_inside_buffer(const GowlGlassPlan *plan, const gchar *what)
{
...
test_whole_window_on_a_1x_output(void)
{
	...
	g_assert_true(gowl_glass_plan(&frame, &mon, 1920, 1080, 1, &plan));
...
test_hidpi_doubles_everything_once(void)
{
	...
	g_assert_true(gowl_glass_plan(&frame, &mon, 2880, 1920, 1, &plan));
...
test_divisor_shrinks_the_buffer_not_the_wallpaper(void)
{
	...
	struct wlr_box frame = { 100, 200, 800, 600 };
	GowlGlassPlan  full, half;

	g_assert_true(gowl_glass_plan(&frame, &mon, 2880, 1920, 1, &full));
	g_assert_true(gowl_glass_plan(&frame, &mon, 2880, 1920, 2, &half));
...
test_refusals(void)
{
	...
	g_assert_false(gowl_glass_plan(&frame, &mon, 0, 0, 1, &plan));
	g_assert_false(gowl_glass_plan(&off, &mon, 1920, 1080, 1, &plan));
	g_assert_false(gowl_glass_plan(&tiny, &mon, 1920, 1080, 1, &plan));
	g_assert_false(gowl_glass_plan(&frame, &degen, 1920, 1080, 1, &plan));
	g_assert_false(gowl_glass_plan(NULL, &mon, 1920, 1080, 1, &plan));
	g_assert_false(gowl_glass_plan(&frame, &mon, 1920, 1080, 1, NULL));
}
...
test_staleness(void)
{
	...
	GowlGlassPlan  pa, pb, pc;

	g_assert_true(gowl_glass_plan(&a, &mon, 1920, 1080, 1, &pa));
	g_assert_true(gowl_glass_plan(&b, &mon, 1920, 1080, 1, &pb));
	g_assert_true(gowl_glass_plan(&c, &mon, 1920, 1080, 1, &pc));

	/* Nothing changed. */
	g_assert_false(gowl_glass_render_stale(TRUE, &pa, &pa, 7, 7, 3, 3));
	/* No buffer yet. */
	g_assert_true(gowl_glass_render_stale(FALSE, &pa, &pa, 7, 7, 3, 3));
	/* It moved. */
	g_assert_true(gowl_glass_render_stale(TRUE, &pa, &pb, 7, 7, 3, 3));
	/* It resized. */
	g_assert_true(gowl_glass_render_stale(TRUE, &pa, &pc, 7, 7, 3, 3));
	/* The wallpaper was captured again -- a tag switch. */
	g_assert_true(gowl_glass_render_stale(TRUE, &pa, &pa, 7, 8, 3, 3));
	/* The settings changed -- a config reload. */
	g_assert_true(gowl_glass_render_stale(TRUE, &pa, &pa, 7, 7, 3, 4));
}
...
test_a_subpixel_move_is_not_a_move(void)
{
	...
	b.origin_x = 100.4;
	g_assert_false(gowl_glass_render_stale(TRUE, &a, &b, 1, 1, 1, 1));
	b.origin_x = 100.6;
	g_assert_true(gowl_glass_render_stale(TRUE, &a, &b, 1, 1, 1, 1));
}
...
main(int argc, char **argv)
{
	...
	g_test_add_func("/glass-geom/whole-window", test_whole_window_on_a_1x_output);
	g_test_add_func("/glass-geom/hidpi", test_hidpi_doubles_everything_once);
	g_test_add_func("/glass-geom/divisor",
	                test_divisor_shrinks_the_buffer_not_the_wallpaper);
	g_test_add_func("/glass-geom/overflow",
	                test_window_hanging_off_the_right_edge);
	g_test_add_func("/glass-geom/before-output",
	                test_window_starting_before_the_output);
	g_test_add_func("/glass-geom/refusals", test_refusals);
	g_test_add_func("/glass-geom/staleness", test_staleness);
	g_test_add_func("/glass-geom/subpixel",
